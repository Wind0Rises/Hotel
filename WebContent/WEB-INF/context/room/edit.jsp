        	...
        	<div style="padding-left: 60px;border: solid red 1px;">
        		<form class="form-horizontal" id="form_edit">
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-10">
					      	<input class="form-control" id="roomNo" name="roomNo" value="${room.roomNo }" placeholder="请填写房间号">
					    </div>
				  	</div>
				  	<div class="form-group">
				    	<label for="level" class="col-sm-2 control-label">楼层：</label>
					    <div class="col-sm-10">
					      <input type="text" class="form-control" name="level" id="level" value="${room.level }" placeholder="请填写房间的楼层">
					    </div>
					</div>
					<div class="form-group">
				    	<label for="original_price" class="col-sm-2 control-label">房间原件：</label>
					    <div class="col-sm-10">
					      <input type="text" class="form-control" name="original_price" id="original_price" value="${room.original_price }" placeholder="请填写原价">
					    </div>
					</div>
					<div class="form-group">
				    	<label for="now_price" class="col-sm-2 control-label">房间现价：</label>
					    <div class="col-sm-10">
					      <input type="text" class="form-control" name="now_price" id="now_price" value="${room.now_price }" placeholder="请填写现价">
					    </div>
					</div>
					    ...
					    <div class="col-sm-10">
					    	<c:choose>
					    		<c:when test="${room.status != null}">
					    			<input type="radio" id="status" name="status" value="0" <c:if test="${room.status == 0 }">checked</c:if> />空闲
					    			<input type="radio" id="status" name="status" value="1" <c:if test="${room.status == 1 }">checked</c:if> />入住中
					    			<input type="radio" id="status" name="status" value="2" <c:if test="${room.status == 2 }">checked</c:if> />清理中
					    		</c:when>
					    		<c:otherwise>
					    			<input type="radio" id="status" name="status" value="0" checked />空闲
					    			<input type="radio" id="status" name="status" value="1"  />入住中
					    			<input type="radio" id="status" name="status" value="2"  />清理中
					    		</c:otherwise>
					    	</c:choose>
					    </div>
					</div>
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-10">
					      	<textarea class="form-control" name="description" name="description" rows="3" placeholder="描述（备注信息）">${room.description }</textarea>
					    </div>
					</div>
